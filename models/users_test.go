...
func testUserToManyUserContacts(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, userContactDBTypes, false, userContactColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, userContactDBTypes, false, userContactColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.UserContacts().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadUserContacts(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserContacts); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.UserContacts = nil
	if err = a.L.LoadUserContacts(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserContacts); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpUserContacts(t *testing.T) {
	var err error
	...
	var a User
	var b, c, d, e UserContact

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*UserContact{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, userContactDBTypes, false, strmangle.SetComplement(userContactPrimaryKeyColumns, userContactColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*UserContact{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddUserContacts(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.UserContacts[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.UserContacts[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.UserContacts().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	userDBTypes = map[string]string{`ID`: `int`, `Email`: `varchar`, `Status`: `enum('active','inactive')`, `CreatedAt`: `datetime`, `ModifiedAt`: `datetime`}
	_           = bytes.MinRead
)
