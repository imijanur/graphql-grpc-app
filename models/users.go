...
var UserWhere = struct {
	...
	ModifiedAt: whereHelpertime_Time{field: "`users`.`modified_at`"},
}
...
type userR struct {
	...
	UserContacts  UserContactSlice `boil:"UserContacts" json:"UserContacts" toml:"UserContacts" yaml:"UserContacts"`
}
...
func (r *userR) GetUserContacts() UserContactSlice {
	if r == nil {
		return nil
	}
	return r.UserContacts
}
...
var (
	userAllColumns            = []string{"id", "email", "status", "created_at", "modified_at"}
	userColumnsWithoutDefault = []string{"email"}
	userColumnsWithDefault    = []string{"id", "status", "created_at", "modified_at"}
...
// UserContacts retrieves all the user_contact's UserContacts with an executor.
func (o *User) UserContacts(mods ...qm.QueryMod) userContactQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
...
func (userL) LoadUserAddresses(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`user_address.user_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// LoadUserContacts allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUserContacts(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`user_contact`),
		qm.WhereIn(`user_contact.user_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.UserContacts = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.UserContacts = append(local.R.UserContacts, foreign)
				if foreign.R == nil {
					foreign.R = &userContactR{}
				}
				foreign.R.User = local
...
// AddUserContacts adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UserContacts.
// Sets related.R.User appropriately.
func (o *User) AddUserContacts(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UserContact) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
	...
	if o.R == nil {
		o.R = &userR{
			UserContacts: related,
		}
	} else {
		o.R.UserContacts = append(o.R.UserContacts, related...)
	}
